    ...
    return (
        <div>
            ...
            <div className="relative z-20 flex items-center  justify-end w-full h-screen">
                <div className={`transition-transform duration-1000 transform ${isLogin ? 'rotate-y-0' : 'rotate-y-180'} w-1/2 h-full flex items-center justify-center`}>

                    {/* ด้านหน้า */}
                    <div className={`absolute ml-64 lg:ml-28 w-9/12 h-3/4 backface-hidden flex justify-center items-center ${isLogin ? '' : 'hidden'}`}>
                        <div className="flex justify-evenly flex-col w-2/3 h-3/4 bg-white bg-opacity-60 shadow-lg p-10 rounded-3xl">
                            ...
                            <div className="text-base flex gap-1 justify-center">
                                ...
                                <button onClick={toggleComponent} className="underline hover:text-my-prim font-bold">Register</button>
                            </div>
                        </div>
                    </div>

                    {/* ด้านหลัง */}
                    <div className={`absolute ml-64 lg:ml-36 w-9/12 h-3/4 backface-hidden flex justify-center items-center rotate-y-180 ${isLogin ? 'hidden' : ''}`}>
                        <div className="flex justify-evenly flex-col w-2/3 h-3/4 lg:h-5/6 lg:w-3/4 bg-white bg-opacity-60 shadow-lg p-10 rounded-3xl">
                            ...
                            <div className=" flex text-lg gap-1 justify-center">
                                ...
                                <button onClick={toggleComponent} className="underline hover:text-my-prim font-bold">Log in</button>
                            </div>
                        </div>
                    </div>
